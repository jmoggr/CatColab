  ...
  security.sudo.extraRules = [
    {
      ...
      commands = [
        {
          command = "/run/current-system/sw/bin/systemctl start automerge";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/run/current-system/sw/bin/systemctl stop automerge";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/run/current-system/sw/bin/systemctl restart automerge";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/run/current-system/sw/bin/systemctl start backend";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/run/current-system/sw/bin/systemctl stop backend";
          options = [ "NOPASSWD" ];
        }
        {
          command = "/run/current-system/sw/bin/systemctl restart backend";
          options = [ "NOPASSWD" ];
        }
        {
          command = "${updateScript}/bin/catcolab-update";
          options = [ "NOPASSWD" ];
        }
      ];
    }
